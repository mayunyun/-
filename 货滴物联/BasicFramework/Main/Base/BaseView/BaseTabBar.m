...
- (void)setWithSeccessView{
    ...
    UIButton *butSY = [[UIButton alloc]initWithFrame:CGRectMake(20*MYWIDTH, 120*MYWIDTH, imageview.width-40*MYWIDTH, 40*MYWIDTH)];
    [butSY setTitle:@"货滴同城" forState:UIControlStateNormal];
    ...
    UIButton *butBJ = [[UIButton alloc]initWithFrame:CGRectMake(20*MYWIDTH,butSY.bottom+15*MYWIDTH, imageview.width-40*MYWIDTH, 40*MYWIDTH)];
    [butBJ setTitle:@"货滴搬家" forState:UIControlStateNormal];
    [butBJ setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    butBJ.titleLabel.font = [UIFont systemFontOfSize:20*MYWIDTH];
    butBJ.backgroundColor = [UIColor whiteColor];
    butBJ.layer.masksToBounds = YES;
    butBJ.layer.cornerRadius = 5;
    butBJ.layer.borderColor = [UIColor blackColor].CGColor;//设置边框颜色
    butBJ.layer.borderWidth = 1;//设置边缘宽度
    [butBJ setImage:[UIImage imageNamed:@"右箭头"] forState:UIControlStateNormal];
    butBJ.titleEdgeInsets = UIEdgeInsetsMake(0, -5*MYWIDTH, 0, 0);
    butBJ.imageEdgeInsets = UIEdgeInsetsMake(0, labelWidth + space + 25*MYWIDTH, 0,  -labelWidth - space);
    [butBJ addTarget:self action:@selector(BJbutHideClick) forControlEvents:UIControlEventTouchUpInside];
    [imageview addSubview:butBJ];
    
    UIButton *butKY = [[UIButton alloc]initWithFrame:CGRectMake(20*MYWIDTH,butBJ.bottom+15*MYWIDTH, imageview.width-40*MYWIDTH, 40*MYWIDTH)];
