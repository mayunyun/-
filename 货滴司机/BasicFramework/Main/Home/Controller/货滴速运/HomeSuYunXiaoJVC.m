...
@interface HomeSuYunXiaoJVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation HomeSuYunXiaoJVC
{
    NSInteger _page;
    ...
    [[RequestTool sharedRequestTool].userRequestTool loginWithUrl:URLStr Parameters:params FinishedLogin:^(id responseObject) {
        ...
        if ([[diction objectForKey:@"rows"] count]) {
            for (NSDictionary *dic in [diction objectForKey:@"rows"]) {
                //建立模型
                ZHXiaoJModel *model=[[ZHXiaoJModel alloc]init];
...
- (UITableView *)tableview{
    if (_tableview == nil) {
        ...
        [_tableview registerClass:[ZHXiaoJTableViewCell1 class] forCellReuseIdentifier:NSStringFromClass([ZHXiaoJTableViewCell1 class])];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    Class ZuLinCarClass = [ZHXiaoJTableViewCell1 class];
    ZHXiaoJTableViewCell1 *cell = nil;
    ...
    if (_dataArr.count) {
        NSLog(@"%@",_dataArr);
        ZHXiaoJModel *model = _dataArr[indexPath.row];
